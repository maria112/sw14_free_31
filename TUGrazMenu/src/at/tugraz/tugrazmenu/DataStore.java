...
public class DataStore {
	...
	private static void fetchRssData(List<Restaurant> restaurants,
			List<MenuItem> menuItemList) throws IOException,
			ParserConfigurationException, SAXException,
			java.text.ParseException {
		XmlHelper xmlHelper = new XmlHelper();
		String content = xmlHelper
				.getSiteContent("http://rss.tugraz.at/menue.xml");
		NodeList nodes = xmlHelper.splitBodyIntoSingleItems(content, "item");
		List<MenuItemContainer> menuItemContainers = xmlHelper
		...
		for (MenuItemContainer menuItemContainer : menuItemContainers) {
			converter.convertAndAddToList(menuItemContainer, menuItemList,
					restaurants);
		}
	}

	public static void fetchHtmlData(List<Restaurant> restaurantList,
			List<MenuItem> menuItemList) throws Exception {
		...
		GregorianCalendar startDate = parser
				.stringToGregorianCalendar(firstDateString);
		List<Restaurant> currentWeekRestaurants = new ArrayList<>();
		List<MenuItem> currentWeekMenus = new ArrayList<>();

		parser.populateItemAndRestaurantLists(startDate, content,
				currentWeekRestaurants, currentWeekMenus);
		GregorianCalendar today = new GregorianCalendar();
		boolean todayMenus = false;
		int i = 0;
		while (!todayMenus && i < menuItemList.size()) {
			if (today.get(Calendar.ERA) == menuItemList.get(i).date
					.get(Calendar.ERA)
					&& today.get(Calendar.YEAR) == menuItemList.get(i).date
							.get(Calendar.YEAR)
					&& today.get(Calendar.DAY_OF_YEAR) == menuItemList.get(i).date
		...
		for (MenuItem currentWeekMenu : currentWeekMenus) {
			if (currentWeekMenu.date.get(Calendar.ERA) == today
					.get(Calendar.ERA)
					&& (currentWeekMenu.date.get(Calendar.YEAR) > today
							.get(Calendar.YEAR) || (currentWeekMenu.date
							.get(Calendar.YEAR) == today.get(Calendar.YEAR) && currentWeekMenu.date
	...
	private static void fetchData(List<Restaurant> restaurants,
			List<List<MenuItem>> restaurantMenus) throws Exception {
