...
public class HtmlParser extends XmlHelper {
	...
	public GregorianCalendar stringToGregorianCalendar(String dateString)
	...
	public void populateItemAndRestaurantLists(GregorianCalendar startDate,
			String htmlContent, List<Restaurant> restaurants,
			List<MenuItem> menuItems) throws Exception {
		...
		for (int i = 1; i < 6 && i < contentSplitByDate.length; i++) {
			...
			for (int j = 1; j < splitByRestaurant.length; j++) {
				...
				int restaurantLocation = restaurantChecker
						.existsInRestaurantListAtIndex(restaurants,
								currentRestaurant);
				if (restaurantLocation > -1) {
					currentRestaurant = restaurants.get(restaurantLocation);
				} else {
					temp = htmlContent.split(Pattern
							.quote(currentRestaurant.name + " ("));
					if (temp.length > 1) {
						currentRestaurant.address = temp[1].split(",")[0];
						currentRestaurant.telephoneNumber = htmlContent
								.split(Pattern.quote(currentRestaurant.name
										+ " ("))[1].split("Tel.")[1]
								.split("\\)")[0].trim();

						currentRestaurant.name = StringEscapeUtils
								.unescapeHtml4(currentRestaurant.name);
						currentRestaurant.address = StringEscapeUtils
								.unescapeHtml4(currentRestaurant.address);
				...
				String menuItemDescriptions[] = splitByRestaurant[j]
						.split("<li>");
				for (int k = 1; k < menuItemDescriptions.length; k++) {
					String description = menuItemDescriptions[k].split("</li>")[0];
					MenuItem currentItem = new MenuItem(currentRestaurant,
							description, dates.get(i - 1));
					currentItem.content = StringEscapeUtils
							.unescapeHtml4(currentItem.content);
	...
	public int menuItemExistsInListAt(List<MenuItem> menuItemList,
			MenuItem currentItem) {

		int index = -1;
		for (int i = 0; i < menuItemList.size(); i++) {
			if (menuItemList.get(i).content.equals(currentItem.content)
					&& menuItemList.get(i).date == currentItem.date
					&& menuItemList.get(i).restaurant.name
							.equals(currentItem.restaurant.name)) {
				index = i;
			}
		}
		return index;
