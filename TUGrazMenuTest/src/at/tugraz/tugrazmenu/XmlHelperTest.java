...
public class XmlHelperTest extends TestCase {
    ...
    public void testSplitBodyIntoSingleItems() throws Exception {
        ...
        NodeList items = xmlHelper.splitBodyIntoSingleItems(inputString, "item");

        if (items.getLength() > 0) {
            assertEquals(2, items.getLength());
            assertEquals("item2", items.item(1).getChildNodes().item(0).getTextContent());
    ...
    public void testCreateMenuList() throws Exception {
        ...
        String xmlFormattedString = " <item> <title>"
        ...
        NodeList nodes = xmlHelper.splitBodyIntoSingleItems(xmlFormattedString, "item");

        List<MenuItemContainer> itemContainers = xmlHelper.createMenuList(nodes);
