...
public class HtmlParserTest extends TestCase {
	...
	public void testWeekDates() throws Exception {
		...
		List<GregorianCalendar> weekDates = parser
				.getWeekDates(new GregorianCalendar(2013, 0, 1));
		for (int i = 0; i < 5; i++) {
			Assert.assertEquals(new GregorianCalendar(2013, 0, i + 1),
					weekDates.get(i));
		}
	}
	...
	public void testPopulateItemAndRestaurantLists() throws Exception {
		...
		parser.populateItemAndRestaurantLists(
				new GregorianCalendar(2003, 3, 3), content, restaurantList,
				menuItemList);
		...
		Assert.assertEquals("0316-873/5799",
				restaurantList.get(2).telephoneNumber);
		Assert.assertEquals(
				"Tagessuppe, Spaghetti Bolognese und Salat [EUR 6,50]",
				menuItemList.get(0).content);
		Assert.assertEquals("Galileo", menuItemList.get(0).restaurant.name);
	}

	public void testMenuItemExistsInListAt() {
		List<MenuItem> menuItemList = new ArrayList<>();
		MenuItem itemInList = new MenuItem(
				new Restaurant("inList", null, null), "menu1",
				new GregorianCalendar(2001, 1, 1));
		MenuItem itemNotInList = new MenuItem(new Restaurant("inList", null,
				null), "menu1", new GregorianCalendar(2001, 2, 1));
		menuItemList.add(itemInList);
		HtmlParser parser = new HtmlParser();
		Assert.assertEquals(0,
				parser.menuItemExistsInListAt(menuItemList, itemInList));
		Assert.assertEquals(-1,
				parser.menuItemExistsInListAt(menuItemList, itemNotInList));
	}
}