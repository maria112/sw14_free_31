...
public class MenuActivityTest extends
	...
	public void testLoadData() {
		...
		FragmentPagerAdapter adapter = (FragmentPagerAdapter) pager
				.getAdapter();
		PageToday pageMonday = (PageToday) adapter.getItem(0);

		pageMonday.loadMenus(restaurants, menusList);

		listMenus = (ExpandableListView) pageMonday.getView().findViewById(
				R.id.listMenuItems);

		assertEquals(2, listMenus.getCount());
		for (int i = 0; i < restaurants.size(); i++) {
			ViewGroup itemview = (ViewGroup) listMenus
					.getExpandableListAdapter().getGroupView(i, false, null,
							null);
			TextView txtRestaurant = (TextView) itemview
					.findViewById(R.id.textRestaurant);
			assertEquals(restaurants.get(i).name, txtRestaurant.getText());

			for (int j = 0; j < menusList.get(i).size(); j++) {
				ViewGroup menuview = (ViewGroup) listMenus
						.getExpandableListAdapter().getChildView(i, j, false,
								null, null);
				TextView txtMenu = (TextView) menuview
	...
	public void testPageAdapter() {
		...
		TextView weekday = (TextView) pageTuesday.getView().findViewById(
				R.id.textrestaurant);
	...
	public void testFilterMenus() {
		...
		pageMonday.filterMenus(allRestaurants, allRestaurantMenus,
		...
		assertEquals(allRestaurantMenus.get(0).get(0), currentRestaurantMenus
				.get(0).get(0));
		assertEquals(allRestaurantMenus.get(1).get(1), currentRestaurantMenus
				.get(1).get(0));
